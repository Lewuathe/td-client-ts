...
describe('TD Client', () => {
  ...
  nock('http://api.treasuredata.com')
      ...
      jobs: [
        {
          ...
          user_name: 'kaisasaki',
          result: null,
          url: 'http://console.treasuredata.com/jobs/1234',
  ...
  it('should get list of database', async () => {
    const ret = await td.listDatabases();
    expect(ret.databases).to.have.length(2);
    expect(ret.databases[0]).to.haveOwnProperty('name');
  });

  it('should create a database', async () => {
    const ret = await td.createDatabase('test_db');
    expect(ret.database).to.be.equal('test_db');
  });

  it('should delete a database', async () => {
    const ret = await td.deleteDatabase('test_db');
    expect(ret.database).to.be.equal('test_db');
  });

  it('should get job list', async () => {
    const ret = await td.listJobs();
    expect(ret.jobs).to.have.length(2);
    expect(ret.jobs[0]).to.haveOwnProperty('query')
      .with.equal('SELECT * FROM www_access');
  });

  it('should get table list', async () => {
    const ret = await td.listTables('test_db');
    expect(ret.database).to.be.equal('test_db');
    expect(ret.tables).to.have.length(2);
  });

  it('should create a table', async () => {
    const ret = await td.createTable('test_db', 'table1', 'log');
    expect(ret.database).to.be.equal('test_db');
  ...
  it('should delete a table', async () => {
    const ret = await td.deleteTable('test_db', 'table1');
    expect(ret.database).to.be.equal('test_db');
    expect(ret.table).to.be.equal('table1');
  });

  it('should get a job', async () => {
    const ret = await td.showJob('1234');
    expect(ret.query).to.be.equal('SELECT * FROM www_access');
  });

  it('should kill the job', async () => {
    const ret = await td.kill('1234');
